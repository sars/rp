    ...
    public function isAllowed($role = null, $resource = null, $privilege = null)
    {
        if (null !== $role) {
        	...
        	if(is_object($role))
        	{
        		...
        		{
        			throw new Zend_Acl_Exception("Role '{$role->getRoleId()}' does not exist");
        		}
        	}
        	else
        	{
        	...
        	// <END ADD>
          // WAS: $role = $this->_getRoleRegistry()->get($role);
        }
        ...
        if (null !== $resource) {
        	...
        	if(is_object($resource))
        	{
        		if(! $this->has($resource))
        		{
        			throw new Zend_Acl_Exception("Resource '{$resource->getResourceId()}' does not exist");
        		}
        	}
        	else
        	{
        	...
        	// <END ADD>
          // WAS: $resource = $this->get($resource);
        }
